...
describe('ContactSection', () => {
  ...
  it('renders contact form', async () => {
    ...
    expect(screen.getByText(/начать проект|start project/i)).toBeInTheDocument();
  });
  ...
  it('shows validation errors for empty fields', async () => {
    ...
    // Wait for animations to complete and reCAPTCHA to be initialized
    await waitFor(() => {
      expect(screen.getByTestId('name-input')).toBeInTheDocument();
    }, { timeout: 10000 });
    
    // Wait a bit more to ensure reCAPTCHA token is set
    ...
    // Wait for validation errors to appear - check for error messages or error role
    await waitFor(() => {
      // Look for elements with role="alert" or error messages
      const errorElements = screen.queryAllByText(/обязательно|required|необходимо|must not be empty|введите|заполните/i);
  ...
  it('submits form with valid data', async () => {
    ...
    // Wait for reCAPTCHA to be initialized and token to be set
    await new Promise(resolve => setTimeout(resolve, 500));
